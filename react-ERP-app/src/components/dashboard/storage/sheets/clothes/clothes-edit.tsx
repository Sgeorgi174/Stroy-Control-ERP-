...
export function ClothesEdit({ clothes }: ClothesEditProps) {
  ...
  const {
    ...
    defaultValues: {
      name: clothes.name,
      providerId: clothes.provider.id,
      clothingSizeId: activeTab === "footwear" ? "" : clothes.clothingSize.id,
      footwearSizeId: activeTab === "clothing" ? "" : clothes.footwearSize.id,
      clothingHeightId:
        activeTab === "footwear" ? "" : clothes.clothingHeight.id,
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await updateClothesMutation.mutateAsync({
        name: data.name.trim(),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        ...
        <div className="flex justify-center mt-10">
          <Button
            type="submit"
            className="w-[300px]"
            disabled={updateClothesMutation.isPending}
          >
            {updateClothesMutation.isPending
              ? "Обновление..."
              : "Обновить одежду"}
          </Button>
        </div>
