...
import {
  useCreateFootwearSize,
  useDeleteFootwearSize,
  useFootwearSizes,
} from "@/hooks/clothes/useClothes";

export function FootwearSizesDialog() {
  const [open, setOpen] = useState(false);
  const [newSize, setNewSize] = useState("");

  const { data: sizes = [], isLoading } = useFootwearSizes();
  const createSize = useCreateFootwearSize();
  const deleteSize = useDeleteFootwearSize();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Размеры обуви</DialogTitle>
        </DialogHeader>
