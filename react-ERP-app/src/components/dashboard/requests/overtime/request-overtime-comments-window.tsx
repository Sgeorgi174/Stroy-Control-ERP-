...
import {
  useAddOvertimeComment,
  useDeleteOvertimeComment,
  useOvertimeComments,
} from "@/hooks/overtime/useOvertimes";
import type { EmployeeOvertimeComment } from "@/types/overtime";
...
export function OvertimeCommentsWindow({ overtimeId, employeeName }: Props) {
  ...
  const { data: comments = [], isLoading } = useOvertimeComments(overtimeId);
  const addMutation = useAddOvertimeComment(overtimeId);
  const deleteMutation = useDeleteOvertimeComment(overtimeId);
              ...
              <div className="p-4 space-y-4">
                {isLoading ? (
                  <div className="flex flex-col items-center justify-center py-20 gap-2 text-muted-foreground">
                    ...
                    <p className="text-xs">Загрузка комментариев...</p>
                  </div>
                ) : comments.length === 0 ? (
                  ...
                  comments.map((comment: EmployeeOvertimeComment) => (
                    <div key={comment.id} className="group flex flex-col gap-1">
                      ...
                      <div
                        className={cn(
                          ...
                          comment.isSystemComment
                            ? "bg-muted/30 border-muted italic text-muted-foreground"
