...
//   const isDateAlreadyLogged = existingLogs
//     .filter((l) => l.id !== log.id)
//     .some((l) => isSameDay(new Date(l.date), selectedDate));
...
//     const total = existingPhotos.length + photos.length + files.length;
//     if (total > 3) return;

//     const updatedFiles = [...photos, ...files];
...
//           <Plus className="h-4 w-4 mr-2" /> Редактировать запись
//         </Button>
//       </DialogTrigger>
...
//                 </div>
//               ))}
//               {existingPhotos.length + previews.length < 3 && (
